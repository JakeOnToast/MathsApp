...
typedef QuestionBuilder = Question Function(int level);
...
Question addQuestionBuilder(final int level){
  ...
  switch(level){
    ...
    case 3:
      ...
      questionString = "$a-$b";
      answerString = answer.toString();
      wrongAnswers = generateWrongAnswers(correctAnswer: answer, minAnswer: max(answer - 4, -2), maxAnswer: min(answer + 4, 12));
      break;
...
Question minusQuestionBuilder(final int level){
  late final String questionString;
  late final String answerString;
  final List<String> choices = [];
  late final List<int> wrongAnswers;
  ...
  switch(level){
    case 0:
      final a = _random.nextInt(6) + 4;
      final b = _random.nextInt(a) + 1;
      final answer = a - b;
      questionString = "$a-$b";
      answerString = answer.toString();
      // fix error when answer is 0 and gives max answer as 0
      wrongAnswers = generateWrongAnswers(correctAnswer: answer, minAnswer: max(answer - 8, -2), maxAnswer: min(3*answer, answer + 5));
      break;
    case 1:
      final a = _random.nextInt(75) + 26;
      final b = _random.nextInt(a - 10);
...
Question multiplyQuestionBuilder(final int level){
  late final String questionString;
  late final String answerString;
  final List<String> choices = [];
  late final List<int> wrongAnswers;
  ...
  switch(level){
    ...
    case 0:
      ...
      questionString = "$a*$b";
      answerString = answer.toString();
      wrongAnswers = generateWrongAnswers(correctAnswer: answer, minAnswer: max(answer - 8, 0), maxAnswer: min(2*answer, answer + 8), stepSize: a~/2);
      break;
